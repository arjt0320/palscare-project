    ...
    const handleAuthCallback = async () => {
      if (isAuthenticated && user) {
        ...
        try {
          ...
          // 3. Query profile status to determine routing
          try {
            const profile = await apiRequest("/api/v1/doctors/profile", "GET", null, "DOCTOR");
            
            // Check if they completed onboarding registration number
            if (profile && profile.registrationNumber && !profile.registrationNumber.startsWith("REG-")) {
      ...
      <div className="z-10 mx-auto w-full max-w-md animate-fade-up">
        ...
        <div className="mt-12 rounded-3xl bg-card/80 p-8 shadow-elevated backdrop-blur-xl border border-border/50 text-center">
          <h2 className="font-display text-2xl font-bold text-foreground">
            {showLoader ? "Authenticating..." : "Doctor Access Portal"}
          </h2>
          <p className="mt-2 text-sm text-muted-foreground">
            {showLoader 
              ? "Please wait while we establish a secure connection."
              : "Access your clinical profile, customize chambers, and manage consultation slots."
            }
          </p>
